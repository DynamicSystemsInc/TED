...
static gchar * lockdown_get_stripped_exec (const gchar *);
static gboolean lockdown_is_allowed_application (const gchar *);
                         ...
                         GError **error)
{
	...
	if (ret) {
 	    ...
 	    g_debug ("success gsm_autostart_app_start(): desktop_id=%s number of launch (%d) launched at %ld", aapp->priv->desktop_id, aapp->priv->launch_counter, aapp->priv->launch_time);
	}
