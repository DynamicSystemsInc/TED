...
class DeleteSnapManager:
    ...
    def __init__(self, snapshots = None):
        ...
        self.xml.set_translation_domain(GETTEXT_DOMAIN)
        self.xml.add_from_file("%s/../../glade/time-slider-delete.glade" \
                                  % (os.path.dirname(__file__)))
        self.backuptodelete = []
        self.shortcircuit = []
        maindialog = self.xml.get_object("time-slider-delete")
        self.pulsedialog = self.xml.get_object("pulsedialog")
        ...
        self.progressdialog = self.xml.get_object("deletingdialog")
        self.progressdialog.set_transient_for(maindialog)
        self.progressbar = self.xml.get_object("deletingprogress")
        # signal dictionary
    ...
    def initialise_view(self):
        if len(self.shortcircuit) == 0:
            ...
            self.snaptreeview = self.xml.get_object("snaplist")
            self.snaptreeview.set_model(self.liststorefs)
            self.snaptreeview.get_selection().set_mode(Gtk.SelectionMode.MULTIPLE)
            ...
            self.typefiltercombo = self.xml.get_object("typefiltercombo")
            ...
            self.schedfilterentry = self.xml.get_object("schedfilterentry")
            ...
            self.schedfilterentry.pack_start(schedentryCell, True)
            ...
            confirm = self.xml.get_object("confirmdialog")
            summary = self.xml.get_object("summarylabel")
    ...
    def __on_filterentry_changed(self, widget):
        ...
        if iter == None:
            filesys = self.fsfilterentry.get_child().get_text()
        else:
        ...
        if iter == None:
            snap = self.schedfilterentry.get_child().get_text()
        else:
    ...
    def __monitor_scan(self):
        if self.snapscanner.isAlive() == True:
            self.xml.get_object("pulsebar").pulse()
            return True
        else:
