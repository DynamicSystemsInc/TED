...
class FileVersionWindow:
    ...
    def __init__(self, snap_path, file):
        ...
        self.builder.add_from_file("%s/../../glade/time-slider-version.glade" \
    ...
    def on_treeview_cursor_changed (self, treeview):
        if self.button_init:
            # code is checking for "text" but type is octet-stream...
...
class VersionScanner(threading.Thread):
    ...
    def run(self):
        l = self.w.snap_path.split (".zfs", 1)
        path_before_snap = l[0]
        l = self.w.filename.split (path_before_snap, 1)
        path_after_snap = l[1]
