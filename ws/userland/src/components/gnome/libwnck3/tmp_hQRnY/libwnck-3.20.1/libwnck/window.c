...
#ifdef TSOL
static void update_window_tsol_role (WnckWindow *window);
#endif
...
const char *
wnck_window_get_tsol_role (WnckWindow *window)
{
  g_return_val_if_fail (WNCK_IS_WINDOW (window), NULL);

  return window->priv->tsol_role;
}
#endif
                               ...
                               WnckWorkspace *space)
{
  ...
  if ((_wnck_check_xtsol_extension) && (_wnck_use_trusted_extensions ())) {
	...
	windowrole = wnck_window_get_tsol_role (window);
    /*
                             ...
                             WnckWorkspace *workspace)
{
  ...
  if (_wnck_use_trusted_extensions () && _wnck_check_xtsol_extension ()) {
	char *windowrole = g_strdup (wnck_window_get_tsol_role (window));
...
static void
update_window_tsol_role (WnckWindow *window)
{
  int error;
  gulong xid;
  uid_t uid = -1;
  ...
  if ((error < 0) || (uid < 0)) {
    ...
    pwd = getpwuid (uid);
  window->priv->tsol_role = g_strdup (pwd->pw_name);
}
...
wnck_window_update_label (WnckWindow *window)
{
  ...
  if (_wnck_check_xtsol_extension () && _wnck_use_trusted_extensions()) {
      if (window->priv->label == NULL) {
	     ...
	     update_window_tsol_role (window);
